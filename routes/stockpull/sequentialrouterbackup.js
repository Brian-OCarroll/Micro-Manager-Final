...
function getStocksFromApi(req, res) {
    ...
    const options2 = {
        method: 'GET',
        url:`https://financialmodelingprep.com/api/company/profile/${req.query.symbol.toLowerCase()}`,
        json:true,
    ...
    .then(function(data) {
        ...
        let parentCompany = parsedSymbolResults[`${req.query.symbol.toLowerCase()}`]["companyName"];
        let companyDescription= parsedSymbolResults[`${req.query.symbol.toLowerCase()}`]["description"];
        let companyImage= parsedSymbolResults[`${req.query.symbol.toLowerCase()}`]["image"];
